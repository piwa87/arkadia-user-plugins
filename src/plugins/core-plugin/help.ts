...
export function setupHelpAliases(api: PluginApi): void {
  const printHelp = () => {
    ...
    const rows: HelpEntry[] = [
      ...
      { cmd: 'pj <text>', desc: 'open room container, przejrzyj <text>' },
      { cmd: 'we <what>', desc: 'open room container, take from it' },
      { cmd: 'wl <what>', desc: 'open room container, put into it' },
      { cmd: 'pj+ <m>|<d>', desc: 'bind container to this room (kufer|kufra)' },
      { cmd: 'pj- / pj?', desc: 'unbind room container / show current' },
      ...
      { cmd: 'work / wgob / wcam', desc: 'take configured loot and fill the worn bag' },
      { cmd: 'whas', desc: 'take HAS loot and fill worn backpack + sack' },
      { cmd: 'w <ork|gob|cam|has>', desc: 'take loot for the selected group' },
      { cmd: 'spork / spgob / spcam / sphas', desc: 'sell loot for the selected group' },
      { cmd: 'sp <ork|gob|cam|has>', desc: 'sell loot for the selected group' },
      { cmd: 'sall', desc: 'run the loot-selling sequence' },
      { cmd: 'zlom+ / zlom-', desc: 'enable / disable scrap equipment mode' },
      { cmd: 'zlomuj', desc: 'take everything, drop remains, fill worn bag' },
      { cmd: 'groups', desc: 'show configured loot groups' },
      ...
      { section: 'WALKER / ZC + PUSTKOWIA' },
      { cmd: 'wk', desc: 'show saved location shortcuts' },
      { cmd: 'wk_all', desc: 'show all shortcuts, including unreachable ones' },
      { cmd: 'wk <shortcut>', desc: 'walk to a saved location with the client walker' },
      { cmd: 'wk <shortcut> !', desc: 'walk to a saved location with the client walker' },
      ...
      { cmd: 'i1–5', desc: 'movement speed presets' },
