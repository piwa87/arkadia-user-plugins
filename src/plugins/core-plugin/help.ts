...
export function setupHelpAliases(api: PluginApi): void {
  const printHelp = () => {
    ...
    const rows: HelpEntry[] = [
      ...
      { section: 'DYNAMIC WALKER / ZC + PUSTKOWIA' },
      { cmd: 'wk', desc: 'show saved location shortcuts' },
      { cmd: 'wk_all', desc: 'show all shortcuts, including unreachable ones' },
      { cmd: 'wk <shortcut>', desc: 'walk normally, or set a dynamic target in ZC/Pustkowia' },
      { cmd: 'wk <shortcut> !', desc: 'start the dynamic walker immediately at i5 speed' },
      { cmd: 'wk+ <key> [label]', desc: 'save or update a character and client shortcut' },
      { cmd: '/zcwalk <roomId>', desc: 'set a manual ZC walker target' },
      { cmd: 'step!', desc: 'diagnose and take one step toward the ZC target' },
      { cmd: 'step!!', desc: 'start the automatic ZC walker at i5 speed' },
      { cmd: '/zcstop', desc: 'clear the ZC walker target' },
      ...
      { cmd: 'prr', desc: 'stop current action, herb route, and ZC walker' },
