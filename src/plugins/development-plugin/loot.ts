...
export function setupLootAliases(api: PluginApi): void {
  ...
  for (let n = 1; n <= 8; n++) {
    registerDev(api, new RegExp(`^w${n}$`, 'i'), `w${n}`, `weź wszystko z trupy #${n}`, () => {
      api.command.send(`wezmij wszystko z ${n} trupy`, false);
      return true;
    });

    registerDev(api, new RegExp(`^m${n}$`, 'i'), `m${n}`, `weź monety z trupy #${n}`, () => {
      api.command.send(`wezmij monety z ${n} trupy`, false);
      return true;
    });

    registerDev(api, new RegExp(`^b${n}$`, 'i'), `b${n}`, `weź broń z trupy #${n}`, () => {
